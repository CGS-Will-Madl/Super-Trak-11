...
function HitMap(img){
  ...
  function Car (img) {
  	...
  	this.img.src = img; // Set source path
...
if (playerOptions == "two"){
	player = new Car('player1.png');
	player2 = new Car('player2.png');
	player2.code = "player2";
}
  ...
  function step (car) {
      ...
      var speedAxis = speedXY(car.rotation, car.speed);
		...
		// constantly decrease speed
  		...
  		// keys movements
  		if (keys[key.W])  { car.accelerate(); }
  		if (keys[key.A]){ car.decelerate(); }
  		if (keys[key.S]){ car.steerLeft(); }
  		if (keys[key.D]){car.steerRight(); }
