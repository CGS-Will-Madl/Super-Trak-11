...
function toName() {
	document.getElementsByClassName('no-3')[0].style.animation = 'fadeOut 1s normal forwards';
	document.getElementsByClassName('no-4')[0].style.animation = 'fadeIn 1s normal forwards';
	document.getElementsByClassName('no-3')[0].remove();
}

function startGame() {
  var nameFromInput = document.getElementById('nameInput').value;
  ...
  document.getElementById('laps').style.animation = 'fadeIn 1s normal forwards'
  document.getElementById('mainOverlay').remove();
  
  var startTime, endTime, finalTime;
  ...
  function step (car) {
      ...
      if (car.collisions.bottom.checkpoint(hit)){
        car.checkpoints += 'i';
		document.getElementById('laps').innerHTML = nameFromInput + ": Lap " + Math.round(car.checkpoints.length / 2) + "/3"
        console.log("Player Checkpoints:" + car.checkpoints)
        if (car.checkpoints.length == 7){
		  end();
		  cancelAnimationFrame(frame);
			document.getElementsByClassName('no-5')[0].style.animation = 'fadeIn 1s normal forwards';
			document.getElementById('gameContainer').remove();

			var firstName = document.getElementById('firstPlaceName');
			var firstCarImg = document.getElementById('firstPlaceCar');
			var firstTime = document.getElementById('firstPlaceTime');
			var firstPoints = document.getElementById('firstPlacePoints');
			var secondName = document.getElementById('secondPlaceName');
			var secondCarImg = document.getElementById('secondPlaceCar');

			firstName.innerHTML = car.name;
			firstCarImg.innerHTML = '<img src="player1.png" />';
			firstTime.innerHTML = finalTime + " seconds";
			firstPoints.innerHTML = (100 - Math.round(finalTime)) + "pts";
			secondName.innerHTML = ai.name;
			secondCarImg.innerHTML = '<img src="player2.png" />';
        }
      }
  	} else if (car.code === 'AI') {
		...
		if (car.collisions.bottom.checkpoint(hit)){
			...
			if (car.checkpoints.length == 7){
			  ...
			  cancelAnimationFrame(frame);
				...
				firstName.innerHTML = car.name;
				firstCarImg.innerHTML = '<img src="player2.png" />';
				firstTime.innerHTML = finalTime + " seconds";
				firstPoints.innerHTML = (100 - Math.round(finalTime)) + "pts";
				secondName.innerHTML = player.name;
				secondCarImg.innerHTML = '<img src="player1.png" />';
			}
		}
    }
  }
